...
public class CalculateController {
    ...
    public CalculatePackagesResponse calculate(@Valid @RequestBody CalculatePackagesRequest request) {
        List<OuterDimensionPack> outerDimensionPacks = request.packages().stream()
                .map((CargoPackage t) -> new OuterDimension(OuterDimension
                                        .add(t.length(),
                                             t.width(),
                                             t.height()).OuterDimension()))
                .map(OuterDimensionPack::new).toList();
        List<Pack> packsWeights = request.packages().stream()
                .map(CargoPackage::weight)
                .map(Weight::new)
                .map(Pack::new)
                .toList();
        var destination = distanceFactory.createDestination(request.destination().latitude(),
                                                            request.destination().longitude());
        var departure = distanceFactory.createDeparture(request.departure().latitude(),
                                                        request.departure().longitude());
        Shipment shipment = new Shipment(packsWeights,
                                         outerDimensionPacks,
                                         currencyFactory.create(request.currencyCode()));
        var calculatedPrice = tariffCalculateUseCase.calculate(shipment, destination, departure);
